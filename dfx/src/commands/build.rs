    ...
    T: BinaryResolverEnv,
{
    ...
    match input_path.extension().and_then(OsStr::to_str) {
        ...
        Some("as") => {
            ...
            env.get_binary_map()
                .idl_compiler()?
                .input(output_idl_path.as_path())
                .output(output_js_path.as_path())
                .build_js()
                .exec()?;
...
mod tests {
    ...
    //    mock! {
    //        pub Env {}
    //        trait BinaryResolverEnv {
    //            fn get_binary_map(&self) -> BinaryMap;
    //        }
    //    }
    ...
    //        env.expect_get_binary_map().times(3);
    //
    ...
    fn build_file_wat() -> () {
        ...
        impl BinaryResolverEnv for TestEnv {
            fn get_binary_map(&self) -> &'_ BinaryMap {
                panic!("binary_map should not be called.")
            }
        }
