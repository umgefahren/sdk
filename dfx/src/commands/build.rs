...
fn didl_compile<T: BinaryResolverEnv>(env: &T, input_path: &Path, output_path: &Path) -> DfxResult {
    ...
    let output = env
        ...
        .arg(&stdlib_path.as_path())
        .output()?;
    err_on_command_failure(
        output,
        |m| DfxError::BuildError(BuildErrorKind::IdlGenerationError(m))
    )
}
...
pub fn err_on_command_failure<F>(output: Output, to_err: F) -> DfxResult
    where F: FnOnce(String) -> DfxError
{
    if !output.status.success() {
        Err(to_err(
            String::from_utf8_lossy(&output.stdout).to_string()
